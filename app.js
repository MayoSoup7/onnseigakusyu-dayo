...
async function addPractice(pageId){
  ...
  $('practiceAudioList').innerHTML=audios.length?audios.map((a,i)=>`<label class="audioPick"><input type="checkbox" value="${a.id}"><span class="audioPickNo">${i+1}</span><span>${esc(a.name)}</span></label>`).join(''):'<div class="empty">先に「＋音声」で音声を登録してください。</div>';
...
async function addAudios(files){if(!files?.length)return;const audios=await all('audios');let order=audios.length;for(const f of files){await put('audios',{id:uid(),lesson:null,order:order++,name:f.name,blob:f})}$('audioFiles').value='';await renderCurrentView();await showAudioManager()}
async function showAudioManager(){const list=(await all('audios')).sort((a,b)=>a.order-b.order);$('audioList').innerHTML=list.length?list.map((a,i)=>`<div class="audioRow"><span class="audioOrder"><input type="number" min="1" value="${i+1}" onchange="setAudioOrder('${a.id}',this.value)"></span><span class="name">${esc(a.name)}</span><div><button onclick="moveAudio('${a.id}',-1)">↑</button><button onclick="moveAudio('${a.id}',1)">↓</button><button onclick="playAudioById('${a.id}')">▶️</button><button class="danger" onclick="deleteAudio('${a.id}')">🗑</button></div></div>`).join(''):'<div class="empty">まだ音声がありません。</div>';$('audioDialog').showModal()}
async function reorderAudios(list){for(let i=0;i<list.length;i++){list[i].order=i;list[i].lesson=null;await put('audios',list[i])}}
async function moveAudio(id,dir){const list=(await all('audios')).sort((a,b)=>a.order-b.order);const i=list.findIndex(a=>a.id===id),j=i+dir;if(i<0||j<0||j>=list.length)return;[list[i],list[j]]=[list[j],list[i]];await reorderAudios(list);await showAudioManager()}
async function setAudioOrder(id,value){const list=(await all('audios')).sort((a,b)=>a.order-b.order);const from=list.findIndex(a=>a.id===id);let to=Math.max(1,Math.min(list.length,Number(value)||1))-1;if(from<0||to===from){await showAudioManager();return}const [item]=list.splice(from,1);list.splice(to,0,item);await reorderAudios(list);await showAudioManager()}
async function renderCurrentView(){if(state.book)await openBook();else await renderLesson()}
...
async function openBook(){
  state.book=true;$('editToolbar').hidden=true;$('pages').hidden=true;$('lessonTitle').innerHTML='';$('audioManager').hidden=true;$('bookView').hidden=false;$('bookBtn').textContent='✏️ 本を編集';
